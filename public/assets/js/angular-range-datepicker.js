...
(function withAngular(angular) {
  ...
  angular.module('720kb.range.datepicker', [])
		.directive('rangedatepicker', ['$window', '$compile', '$locale', '$filter', '$interpolate', function manageDirective($window, $compile, $locale, $filter, $interpolate) {
    ...
    return {
      ...
      'link': function linkingFunction($scope, element, attr) {
        ...
        var selector = attr.selector
          ...
          '<a href="javascript:void(0)" ng-repeat="px in prevMonthDays" ng-class="{\'_720kb-datepicker-active\': shouldHighlightDate(year, monthNumber - 1, px, \'prev\')}" class="_720kb-datepicker-calendar-day _720kb-datepicker-disabled">{{px}}</a>' +
          '<a href="javascript:void(0)" ng-repeat="item in days" ng-click="setDatepickerDay(item)" ng-class="{\'_720kb-datepicker-active\': shouldHighlightDate(year, monthNumber, item, null), \'_720kb-datepicker-disabled\': !isSelectableMinDate(year + \'/\' + monthNumber + \'/\' + item ) || !isSelectableMaxDate(year + \'/\' + monthNumber + \'/\' + item) || !isSelectableDate(monthNumber, year, item)}" class="_720kb-datepicker-calendar-day">{{item}}</a>' +
          '<a href="javascript:void(0)" ng-repeat="nx in nextMonthDays" ng-class="{\'_720kb-datepicker-active\': shouldHighlightDate(year, monthNumber + 1, nx, \'next\')}" class="_720kb-datepicker-calendar-day _720kb-datepicker-disabled">{{nx}}</a>' +
          '</div>' +
          '</div>' +
          '</div>';
        ...
        $scope.setInputValue = function manageInputValue() {
          ...
          if ($scope.isSelectableMinDate($scope.year + '/' + $scope.monthNumber + '/' + $scope.day)
            ...
            if (attr.dateFormat) {
              thisInput.val($filter('date')($scope.date1, dateFormat) + ' - ' + $filter('date')($scope.date2, dateFormat));
            } else {
              thisInput.val(modelDate);
            }

            thisInput.data('start-date', $scope.date1);
            thisInput.data('end-date', $scope.date2);
        ...
        $scope.setDatepickerDay = function setDatepickeDay(day) {
          ...
          if ($scope.isSelectableDate($scope.monthNumber, $scope.year, day)
              ...
              if (!$scope.date1) {
                $scope.date1 = new Date($scope.year + '/' + $scope.monthNumber + '/' + day);
              } else if ($scope.date1 && !$scope.date2){
                $scope.date2 = new Date($scope.year + '/' + $scope.monthNumber + '/' + day);

                if ($scope.date1 > $scope.date2) {
                  var temp = $scope.date1;
        ...
        $scope.shouldHighlightDate = function shouldHighlightDate(year, month, day, thing) {
          ...
          if ($scope.date1 && $scope.date1.getTime() === date.getTime()) {
            //console.log($scope.date1.getTime());
            return true;
          } else if ($scope.date1 && $scope.date2 && date >= $scope.date1 && date <= $scope.date2) {
            return true;
          } 
